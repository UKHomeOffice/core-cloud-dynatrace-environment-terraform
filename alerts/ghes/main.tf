...
resource "dynatrace_metric_events" "freeable_memory_warning_alerts" {
  ...
  event_entity_dimension_key = var.common_ghes_values.event_entity_dimension_key
  summary                    = var.ghes_metrics.memory_usage.summary
  event_template {
    description = var.ghes_metrics.memory_usage.description
    davis_merge = var.common_ghes_values.davis_merge
    event_type  = var.common_ghes_values.event_type
    title       = var.ghes_metrics.memory_usage.warning.title
    metadata {
      metadata_key   = var.common_ghes_values.tag_key
      metadata_value = var.common_ghes_values.tag_value
    }
  }
  model_properties {
    type               = var.common_ghes_values.model_properties_type
    alert_condition    = var.ghes_metrics.memory_usage.alert_condition
    alert_on_no_data   = var.common_ghes_values.alert_on_no_data
    dealerting_samples = var.ghes_metrics.memory_usage.dealerting_samples
    samples            = var.common_ghes_values.samples
    threshold          = var.ghes_metrics.memory_usage.warning.threshold
    violating_samples  = var.common_ghes_values.violating_samples
  }
  query_definition {
    ...
    dimension_filter {
      filter {
        dimension_key   = var.common_ghes_values.dimension_key
        dimension_value = var.common_ghes_values.dimension_value
      }
    }
    entity_filter {
      dimension_key = var.common_ghes_values.dimension_key
      conditions {
        condition {
          type     = var.common_ghes_values.entity_filter_condition1_type
          operator = var.common_ghes_values.entity_filter_condition1_operator
          value    = var.common_ghes_values.entity_filter_condition1_value
        }
      }
    }
  }
