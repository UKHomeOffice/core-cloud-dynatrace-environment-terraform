variable "aws_secrets" {
  type = map(object({
    ...
    tags = optional(map(string),{                     # Tags to be applied to the secret (required by the SCP)
      account-code    = "cosmos"
      cost-centre     = "cosmos"
      service-id      = "srinivasan.sundaram1@digital.homeoffice.gov.uk"
      portfolio-id    = "cosmos"
      project-id      = "cosmos-devel"    
    })
  }))
}